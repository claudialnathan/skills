...
**Where to look when stuck**: the existing codebase and the Tailwind v4 / shadcn docs.
...
This section is here because taste reasons cite frequency and purpose constantly. The same flourish is a gift in one product, friction in another.
...
- **You can't tell when a custom easing is needed vs a stock one** from feel. Reason from what the motion is for and match the project's existing easings; don't pick by numbers alone.
...
3. **Work large asks as sequential narrow passes** — structure → states → polish → motion — each with its own stated reasons, not one combined emission.
