...
- **Use a `<g>` wrapper for cross-browser SVG transforms.** Inside SVG, `transform-origin` can resolve against the viewport box, so a "center" origin rotates the shape around the canvas corner and the animation flies off. Wrap the transformed shape in a `<g>` and set both `transform-box: fill-box` and `transform-origin: center` so the origin resolves against the shape's own bounding box, including in Safari.
