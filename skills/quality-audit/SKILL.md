...
| Design & polish | any UI | `design-polish`, `design-taste`, `design-layout` *(bundled)*, `make-interfaces-feel-better`, `emil-design-eng`, `web-design-guidelines` | concentric radii, `tabular-nums`, `min-h-dvh`, `focus-visible` |
| Motion performance | animations present | `design-motion` *(bundled)*, `fixing-motion-performance`, `framer-motion-animator`, `baseline-ui` | `transform`/`opacity` only on large surfaces; no scroll-driven JS; blur ≤ 8px |
