...
`scripts/preship-check` FAILs on drift in either direction for both the Claude and Cursor manifests (SKILL.md on disk but uncovered; a listed path with no SKILL.md or no directory). The cross-tool mirror is flat — `scripts/sync-cross-tool` collapses `wip/` and `archive/` into one link per skill by name, so skill names must stay globally unique across grouping folders.

## Publishing footgun: Claude stays versionless; Codex does not

The Claude `skills` plugin carries **no `version` field** in `.claude-plugin/plugin.json` or in the Claude marketplace entry that points at this repo. That keeps Claude Code in commit-SHA versioning: every commit reachable from the marketplace's configured source ref is a new version, so a marketplace install picks up skill changes on the next `/plugin update` with no manual bump. A version string there pins Claude's install cache, `/plugin update` then reports "already at the latest version," and pushed changes silently never reach other repos. `scripts/preship-check` fails if one reappears. Propagation after a change reaches that ref: commit → push → `/plugin marketplace update claudia` → `/plugin update skills@claudia`. A feature-branch push can update working-tree mirrors but does not advance the marketplace cache before merge.
